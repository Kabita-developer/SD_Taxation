...
exports.createHR = async (req, res) => {
  try {
    ...
    // Validate role
    if (!role || !['HR', 'OR'].includes(role)) {
      return res.status(400).json({ 
        success: false, 
        message: 'Role must be either HR or OR' 
      });
    }
...
// ✅ Get all HR/OR users controller
...
// ✅ Get HR/OR user by ID controller
