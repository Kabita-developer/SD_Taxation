...
**Description:** Authenticates an HR or OR user and returns a JWT token.
...
**Description:** Logs out the current HR or OR user.
...
**Description:** Creates a new HR or OR user account. Only Admin or SuperAdmin can access this endpoint.
...
```json
{
  ...
  "role": "string (required, must be 'HR' or 'OR')",
...
- `role` (optional): Filter by role (HR or OR)
...
{
  "success": true,
  "message": "HR/OR users retrieved successfully",
  "data": [
    {
      "_id": "64f...abcd",
      "fullname": "Rekha Sharma",
...
### 5. Get HR/OR User by ID

**Endpoint:** `GET /hr/:id`

**Description:** Retrieves a specific HR or OR user by ID.

**Authentication:** Required (Admin or SuperAdmin)

**Parameters:**
- `id`: HR/OR User ID (MongoDB ObjectId)

**Success Response (200):**
```json
{
  "success": true,
  "message": "HR/OR user retrieved successfully",
  "data": {
    "_id": "64f...abcd",
    "fullname": "Rekha Sharma",
    "username": "rekha_hr",
...
**Description:** Updates an existing HR or OR user account.
...
```json
{
  ...
  "role": "string (optional, must be 'HR' or 'OR')"
}
```
...
**Description:** Deletes an HR or OR user account permanently.
...
```javascript
{
  ...
  "role": "String (enum: HR|OR, required)",
  "phone": "String (required, 10-20 characters)",
...
- Only Admin or SuperAdmin roles can manage HR/OR users
- Company references are validated before creating HR/OR users
